...
for user, file_indices in test_set.items():
    for file_idx in file_indices:
        ...
        Prediction_data = dict(np.load(f'/workspace/nmt/Pressure_HPE/test_result/model_test_result/qy_different_train_dataset/{user}_{file_idx}_predictions_6_train_dataset.npz'))
...
for user, file_idx, error, std_dev in mpjpe_results:
    print(f"受试者: {user}, 组号: {file_idx}, MPJPE: {error:.6f}±{std_dev:.6f}")
...
for user, file_idx, error, std_dev in mpjpe_results:
    ...
    for motion_type, condition in motion_type_map.items():
        if condition(user, file_idx):
            # 同时存储平均误差和标准差，并转换为毫米
            organized_data[user][motion_type] = (error * 1000, std_dev * 1000)
...
# 计算每种动作类型的平均值和标准差
for motion_type, errors_with_std in motion_type_errors.items():
    # 从元组列表中只提取平均误差用于计算总的平均值和标准差
    mean_errors = [item[0] for item in errors_with_std]
    ...
    print(f"样本数量: {len(mean_errors)}")
    # 以 "平均值 ± 标准差" 的格式打印每个人的数据

    print("各受试者误差:", [f"{mean:.2f} ± {std:.2f}" for mean, std in errors_with_std])
